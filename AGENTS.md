...
> Gemma 4 E2B (LiteRT-LM), on-device RAG over a verified Bangla corpus, Vosk
> STT, `flutter_map` shelter map, slide-to-confirm 999 dial + SOS SMS. Single
> sentence: **"it works when the internet doesn't."**
...
- **arm64-v8a only.** The LiteRT-LM runtime + `flutter_gemma` are arm64-only. An x86
...
- **No network in the core chat loop.** The whole product thesis is offline. The only
  network call paths are: (1) the one-time ~1.5 GB model download gated by `ModelManager`,
...
core/     Cross-cutting singletons: modelManager, themeController, haptics
...
flutter test                               # 91 pass, 1 skip — see note below
...
`91 passed, 1 skipped`. The `1 skipped` is the intentional `skip: true` in
...
1. `kb.keywordRetriever.topK(query, k: 3)` — **primary path, fully offline**.
...
- `ensureModel()` uses stdlib `HttpClient` with Range-resume; checks for `206` and
  truncates+restarts on `200` to avoid corrupting the file (the comment at line ~109
  explains the gotcha).
...
- Model URL (`ModelManager._defaultUrl`) is a `litert-community` HF mirror of Gemma 4
  E2B IT. If you swap the model, also touch the matching TODO in `_modelFileName` and
  the spike entry in `docs/spike-results.md`.
...
fictional model IDs return 404). The chain is invoked only when `Connectivity` reports
non-`none` results.

**⚠ Security note:** the working tree as of 2026-07-14 contains an uncommitted edit to
`lib/features/chat/chat_screen.dart` that hardcodes a Gemini API key as the
`defaultValue` of `String.fromEnvironment('GEMINI_API_KEY', ...)`. The committed
pattern uses an empty default — passing the key on the command line only. **If you
keep a hardcoded default in code, rotate that key with Google AI Studio immediately**
and remove the `defaultValue`. Never commit any API key.
...
- Permissions required in `AndroidManifest.xml`: `RECORD_AUDIO`, `ACCESS_FINE_LOCATION`,
  `CALL_PHONE`, `SEND_SMS`, plus `INTERNET` (only for the model download).
...
- Bottom nav: `lib/app/main_shell.dart:56`
